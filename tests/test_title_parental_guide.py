...
def test_parental_guide_parser_should_set_advisories(imdb_id, nudity_status):
    ...
    imdb.set_parental_guide(parsed)
    assert parsed.advisories.nudity.status == nudity_status


@pytest.mark.parametrize(("imdb_id", "rating", "reason"), [
    ("tt0133093", "R", "Rated R for sci-fi violence and brief language"),  # The Matrix
    ("tt0468569", "PG-13", "Rated PG-13 for intense sequences of violence and some menace"),  # The Dark Knight
])
...
def test_parental_guide_parser_title_should_set_country_ratings(imdb_id, country, ratings):
    parsed = imdb.get_title(imdb_id=imdb_id)
    imdb.set_parental_guide(parsed)
